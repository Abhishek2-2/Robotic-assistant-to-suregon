...
def callback(data):
  if("blue" in data.data):
    ...
    listener.waitForTransform("/base_link", "/logical_camera_2_unit_box_blue_frame", rospy.Time(), rospy.Duration(10.0))
    (trans, rot) = listener.lookupTransform('base_link', 'logical_camera_2_unit_box_blue_frame', rospy.Time())
  ...
  if("red" in data.data):
    ...
    listener.waitForTransform("/base_link", "/logical_camera_2_package_box_red_frame", rospy.Time(), rospy.Duration(10.0))
    (trans, rot) = listener.lookupTransform('base_link', 'logical_camera_2_package_box_red_frame', rospy.Time())
    current_pose = arm_group.get_current_pose()
    pose_target = geometry_msgs.msg.Pose()
    pose_target.orientation = current_pose.pose.orientation
    pose_target.position.x = trans[0]
